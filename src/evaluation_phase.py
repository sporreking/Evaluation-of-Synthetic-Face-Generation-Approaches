...
def evaluation_phase() -> None:
    ...
    # * Reconstruct factories again based on status
    cmfs = [
        CompoundModelFactory(
            cmf.get_controller(),
            [f for f in filters if filter_status[cmf.get_name()][f.get_name()]],
            SampleMetricRegistry.get_resources(),
            compound_metric_types[cmf.get_name()],
        )
        for cmf in cmfs
...
def _perform_evaluation(
    ...
    for cmf in cmfs:
        ...
        PU.print_list(
            ...
            [f.get_name() for f in cmf.get_filters()],
        )
